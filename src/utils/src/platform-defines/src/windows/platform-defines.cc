...
namespace {
inline constexpr const auto* DLL_SHARED_LIB_FILE_EXTENSION{".dll"};

inline constexpr const auto  UNDECORATED_SYMBOL_BUFFER_SIZE{1024UL};
inline constexpr const auto  LOADED_MODULE_FILE_PATH_BUFFER_SIZE{1024UL};

inline constexpr const auto  UNDECORATE_SYMBOL_RULES_FLAGS{
...
BOOL CALLBACK enumSymbolsCallback(PSYMBOL_INFO pSymInfo, [[maybe_unused]] ULONG SymbolSize, PVOID UserContext)
{
    ...
    symbolsMap->emplace(unDecorateSymbol(static_cast<const char*>(pSymInfo->Name)), pSymInfo->Address);
    return TRUE;
}
